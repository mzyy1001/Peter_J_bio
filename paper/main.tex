...
\noindent
\textbf{Results:} Ground-truth benchmarking against the observed latent hallmark correlations (post age/tissue effects) revealed mixed recovery: weak edge ranking (AUROC = 0.454) but high best-threshold F1 = 0.764, modest centrality recovery ($\rho = 0.433$, top-3 hub overlap = 67\%), poor module recovery (ARI = $-0.169$), strong age-effect recovery ($\rho = 0.796$), and low edge stability across seeds (17\%). The weak AUROC is largely attributable to random signed gene loadings in the simulation that the unsigned scoring method cannot recover---a deliberate stress test. Cross-hallmark prediction showed that antagonistic hallmarks (MD: $R^2 = 0.510$, DNS: $R^2 = 0.486$) were most predictable, while integrative hallmarks (SCE: $R^2 = 0.205$) retained more independent variation. Chronological age prediction from hallmark scores yielded cross-validated MAE = 16.1 years (EN $R^2 = 0.146$). No hallmark pairs showed significant age-dependent correlation changes after FDR correction, consistent with the simulation design which did not embed age-varying covariance structure.

\noindent
\textbf{Conclusions:} We provide an open-source computational framework for dissecting hallmark-level aging interactions, with transparent benchmarking revealing both strengths (best-threshold edge F1, age-effect recovery, cross-hallmark predictability) and limitations (edge-ranking AUROC, module recovery, edge stability). The honest characterization of recovery performance under idealized simulation conditions establishes baseline expectations for future application to real transcriptomic cohorts.
...
The PC algorithm approximation identified 23 oriented edges among the 9 hallmarks (Figure~\ref{fig:causal}). This exploratory analysis yielded orientations that can be compared with the embedded hierarchy, though the approximate implementation (single-variable conditioning, heuristic orientation) limits interpretability. Genomic Instability showed oriented edges toward Telomere Attrition and Cellular Senescence, while Cellular Senescence had an oriented edge toward Altered Intercellular Communication---consistent with known SASP-driven signaling. \textbf{We emphasize that these orientations reflect conditional-dependence structure under strong assumptions, not validated causal relationships.}
...
Random Forest classification achieved 47.7\% accuracy (5-fold CV) in distinguishing three age groups from hallmark scores, compared to 43.3\% for majority-class and stratified-random baselines. Logistic Regression achieved 48.7\%, exceeding both baselines. Gradient Boosting (43.3\%) matched but did not exceed the baselines. The modest improvement of RF and LR over chance indicates that hallmark activity scores carry limited age-discriminative information. Feature importance analysis revealed varying hallmark contributions across models (Figure~\ref{fig:importance}).
...
\subsection{Chronological Age Prediction}
...
Comparison of hallmark correlation networks between young ($<40$, $n=89$) and old ($\geq 60$, $n=130$) groups was performed using Fisher $z$-tests with Benjamini-Hochberg FDR correction across all 36 pairwise comparisons (Figure~\ref{fig:age_changes}). While several pairs showed nominally different correlations, \textbf{no pairs survived FDR correction at $q < 0.05$}. This is consistent with the simulation design, which embeds age main effects on hallmark scores but does not embed age-varying covariance structure (i.e., no differential hallmark-hallmark correlations between young and old). The absence of FDR-significant rewiring is therefore expected, not a power failure. The analysis demonstrates that the FDR-corrected pipeline does not produce false positives for age-dependent rewiring when none is present.
...
\begin{table}[H]
\centering
\caption{Ground-truth benchmarking results. Target: \textbf{observed latent hallmark correlations} (after age and tissue effects, before gene-level noise/scoring). True edges: pairs with $|\rho_{\text{observed}}| > 0.1$. Ground-truth modules: Primary (GI, TA, EA, LP), Antagonistic (DNS, MD, CS), Integrative (SCE, AIC). Note: the simulation uses random signed gene loadings that the unsigned scoring cannot fully recover, deliberately stress-testing the pipeline.}
...
\textbf{Diagnosis of weak AUROC:} The simulation assigns random positive or negative gene loadings, creating mixed up/down signatures within each hallmark gene set. The unsigned ssGSEA scoring method cannot recover the direction of individual gene contributions, producing sign inversions in some hallmark-hallmark correlations. This is a deliberate stress test: it models real-world gene sets where regulation direction varies across genes. The high best-threshold F1 (0.764) indicates that many true edges are detectable at the binary level despite the AUROC weakness.

\textbf{Additional factors:} Shared-gene removal had moderate impact ($r = 0.753$), suggesting some cross-hallmark correlation is driven by overlapping gene sets under the multi-hallmark loading model. Tissue adjustment had negligible effect ($r = 0.999$). The scoring-method comparison showed ssGSEA and mean $z$-score are closely aligned ($r = 0.956$), while sign-anchored PCA also agrees well ($r = 0.783$). Direction-aware methods (GSVA, singscore) may improve edge-ranking AUROC by resolving gene-level sign ambiguity.
...
No FDR-significant age-dependent rewiring was detected, which is the correct result: the simulation embeds age main effects on hallmark scores but does not embed age-varying covariance structure. The null finding demonstrates that the FDR-corrected pipeline avoids false-positive rewiring claims when differential correlations are absent. To validate the pipeline's ability to \textit{detect} age-dependent rewiring, future work should add explicit age-interaction terms to the simulation and assess sensitivity/specificity.

\subsection{Chronological Age Prediction}

The cross-validated chronological age prediction model demonstrated that hallmark activity scores carry some age-predictive information. Elastic Net and Random Forest outperformed baseline predictors in cross-validation, though Gradient Boosting ($R^2 = 0.027$) did not meaningfully exceed baselines. The moderate cross-validated $R^2$ is expected given the noise in ssGSEA scoring and the compression of complex pathway activities into single scalars. Application to real data may yield different performance depending on the quality of hallmark gene sets and the confounding structure of the cohort.
...
\begin{enumerate}[noitemsep]
    ...
    \item Reveal differential cross-hallmark predictability: antagonistic hallmarks (MD, DNS) were most predictable from other hallmark scores, while integrative hallmark SCE retained more independent variation in this simulation;
    \item Evaluate age-dependent network rewiring with FDR-corrected testing; the null finding is consistent with the simulation design (no embedded age-varying covariance);
    \item Predict chronological age from hallmark scores, with Elastic Net partially outperforming mean-age and tissue-only baselines ($R^2 = 0.146$).
\end{enumerate}
